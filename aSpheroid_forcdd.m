...
%     for i=1:size(Z_hi,1)
%        min_val= abs(Z_hi_norm(i,:)-mid_res);
%        tmp=find(min_val==min(min_val));
%        inx_mat(i,1)=i;
%        inx_mat(i,2)=tmp;   
%     end 
%    end
   ...
   plot3(X_hi(inx_mat(:,1)),Y_hi(1,inx_mat(:,2)),norm_z(1,:),'-k','LineWidth',2);
   ...
   area_pixels = 1101 - inx_mat(find(inx_mat(:,2)>100),1)
   area_pixels = sum(area_pixels)
   
   %Plot the Actual Data
   %c=colormap(hot(96));
   c=colormap(redblue(96))
