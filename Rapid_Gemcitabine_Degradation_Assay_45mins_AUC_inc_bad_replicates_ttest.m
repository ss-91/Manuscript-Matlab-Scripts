...
%Read in data for all replicates
DataFileName1 = '2021-09-11-Rapid-Gem-45min-Rep1.xlsx';
DataFileName2 = '2021-09-11-Rapid-Gem-45min-Rep2.xlsx';
DataFileName3 = '2021-09-11-Rapid-Gem-45min-Rep3.xlsx';
DataRange = 'D29:CU149';
...
for r=1:rowNumber;
    for c=1:colNumber;
        ...
        plot(time(1:select_tp),mean_rawwt(1:select_tp,1),'--g','Linewidth',2)
        i = i+1;
        title(Labels(r,c))
    end
end
suptitle({'del-cdd raw growth curves (supernatant 45min)','black=rep1, red=rep2, blue=rep3, green=wt'});
...
for r=1:rowNumber;
    for c=1:colNumber;
        ...
        plot(time(1:select_tp),mean_normwt(1:select_tp,1),'--g','Linewidth',2)
        i = i+1;
        title(Labels(r,c))
    end
end
suptitle({'del-cdd normalized growth curves (supernatant 45min)','black=rep1, red=rep2, blue=rep3, green=wt'});
...
for r=1:8
    for c=1:12
      [h1(r,c) stat_fast_degraders(r,c)]=ttest2(AUC_8x12{r,c},wt_AUC_array,'tail','left');
    end
end
...
%% Color the Slow and Fast Gemcitabine Degraders according to the Growth Rate Determination
...
suptitle({'Significant Slow Degraders(FDR p-adj<' pval,'AUC-' time(select_tp) 'hrs'});
